...
const ChartsTabs = () => {
    ...
    useEffect(() => {
        if (chartsData.length !== 0) {
            let temp = [];
            if (chartsData && chartsData.length === 1) {
                //document.getElementById("addSalaryID").value = chartsData && chartsData.length > 0 && chartsData[0].at(1).at(2);  
                setSalaryDefaultValue(Number(chartsData[0].at(1).at(2)));


                chartsData && chartsData[0].map((item, index) => {
                    if (index !== 0) {
                        temp.push(item[1]);
                    }
                })
               ...
               // document.getElementById("addSalaryID").value = chartsData && chartsData.length > 0 && chartsData[1].at(2); 
    ...
    const deleteChartItems = () => {
        const temp = [deleteCharts.billsDeleteLabel];
        dispatch(deleteChartItem(temp));
    }
